  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!isPro && noOfUsersInChat >= 2) {
      toast({
        ...
        description:
          "You have exceeded the limit of users in a single chat for FREE plan. Please upgrade to PRO to continue adding users to chats!",
        variant: "destructive",
        action: (
          <ToastAction
            altText="Upgrade"
    ...
    if (querySnapshot.empty) {
      ...
      await setDoc(addChatRef(chatId, user.id), {
        ...
        .catch(() => {
          toast({
            title: "Error",
            description: "Error while adding user to chat.",
            variant: "destructive",
          });
